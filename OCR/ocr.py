...
print(f"  - GOOGLE_API_KEY: {'설정됨' if API_KEY else '❌ 설정 안됨'}")
print(f"  - MONGO_URI: {'설정됨' if MONGO_URI else '❌ 설정 안됨'}")
print(f"  - MONGO_DB_NAME: {'설정됨' if MONGO_DB_NAME else '❌ 설정 안됨'}")
print(f"  - MONGO_COLLECTION_NAME: {'설정됨' if MONGO_COLLECTION_NAME else '❌ 설정 안됨'}")
print("="*50)
...
if not MONGO_COLLECTION_NAME:
    missing_vars.append("MONGO_COLLECTION_NAME")

if missing_vars:
    print(f"⚠️ 다음 환경변수가 설정되지 않았습니다: {', '.join(missing_vars)}")
    print(f"📁 .env 파일을 확인하세요: {env_path}")
    print("📖 설정 가이드: OCR-env-setup.txt 참조")
else:
