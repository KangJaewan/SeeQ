...
async def generate_report(
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"보고서 생성 요청 중 오류가 발생했습니다: {str(e)}")
...
async def _generate_report_sync(
    ...
    try:
        ...
        # 선택된 파일들의 내용 조회
        selected_file_ids = [f.file_id for f in selected_files if f.selected]
        documents_content = await db_ops.get_documents_content_by_files(folder_id, selected_file_ids)
        
        file_contents = []
        for i, file_sel in enumerate([f for f in selected_files if f.selected]):
            if i < len(documents_content) and documents_content[i]:
