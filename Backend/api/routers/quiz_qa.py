    ...
    def grade_single_answer(answer: QuizAnswer) -> QuizResult:
        ...
        try:
            ...
            if answer.quiz_type == "multiple_choice":
                # 객관식: user_answer가 텍스트일 수도 있고 인덱스(숫자)일 수도 있음
                user_idx = None
                correct_idx = None

                # user_answer 처리
                try:
                    user_idx = int(answer.user_answer)
                except (ValueError, TypeError):
                    # 텍스트인 경우 options에서 인덱스 찾기
                    if answer.options:
                        try:
                            user_idx = answer.options.index(str(answer.user_answer))
                        except (ValueError, AttributeError):
                            user_idx = None

                # correct_answer 처리
                try:
                    correct_idx = int(answer.correct_answer)
                except (ValueError, TypeError):
                    if answer.options:
                        try:
                            correct_idx = answer.options.index(str(answer.correct_answer))
                        except (ValueError, AttributeError):
                            correct_idx = None
                ...
                # OX 문제: user_answer가 텍스트일 수도 있고 인덱스일 수도 있음
                user_value = None
                correct_value = None
                ...
                try:
                    user_idx = int(answer.user_answer)
                    if answer.options and 0 <= user_idx < len(answer.options):
                        user_value = str(answer.options[user_idx]).lower()
                    else:
                        user_value = str(answer.user_answer).lower()
                except (ValueError, TypeError):
                    user_value = str(answer.user_answer).lower()
                ...
                try:
                    correct_idx = int(answer.correct_answer)
                    if answer.options and 0 <= correct_idx < len(answer.options):
                        correct_value = str(answer.options[correct_idx]).lower()
                    else:
                        correct_value = str(answer.correct_answer).lower()
                except (ValueError, TypeError):
                    correct_value = str(answer.correct_answer).lower()

                # 불린 값으로 변환
                user_bool = user_value in ['true', 'o', '참', '1', 'yes', '맞다', '정답']
                correct_bool = correct_value in ['true', 'o', '참', '1', 'yes', '맞다', '정답']
